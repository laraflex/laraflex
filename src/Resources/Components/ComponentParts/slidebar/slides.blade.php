...
<div class="frame js_frame p-0 m-0">
    ...
    @if (!empty($item->url))
        ...
        @elseif(!empty($item->media) && $item->media == 'vimeo')
            ...
            @endphp
        <a href="{{$item->url}}" data-remote="{{$player}}" data-toggle="lightbox" data-size="lg" data-gallery="media" class=" m-0">
        @endif
        @else
        <a>
        @endif
    ...
    <div class="p-2 w-100 m-0" style="white-spacex: pre-line; background-color: rgb(0,0,0,0.4); font-size: calc(0.6em + 0.4vw); line-height: 1.1;position:relative;">
